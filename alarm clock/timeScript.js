...
var hourDisplay = document.getElementById("displayHours");
var minuteDisplay = document.getElementById("displayMinutes")
var secondDisplay = document.getElementById("displaySeconds")
...
function setTimeFunction(){
	...
	inputHour ? now.setHours(inputHour) : now.getHours();
	inputMinute ? now.setMinutes(inputMinute) : now.getMinutes();
...
function timePrint(){
	...
	standardTime ? hourString = hourFormat() : hourString;

	hourDisplay.innerHTML =  hourString;
	minuteDisplay.innerHTML =  minuteString;
	secondDisplay.innerHTML = secondString;
...
function hourFormat() {
	...
	let standardHourString;

	 

	if (parseInt(hourString) > 12) {
		standardHourString = timeFormat(hour - 12);
	} else if (parseInt(hourString) === 0){
		standardHourString = "12";
	} 

	standardTime = true;

	return standardHourString;
